...
public class StartCommand {
                        ...
                        .then(ClientCommandManager.argument("threads", IntegerArgumentType.integer(1, MAX_THREADS))
                                .executes(context -> {
                                    ...
                                    if (player == null || world == null) {
                                        context.getSource()
                                                .sendError(Text.literal("Command must be run by a player in a world."));
                                        return 0;
                                    }

                                    if (!world.getRegistryKey().getValue().getPath().endsWith("the_nether")) {
                                        context.getSource().sendError(
                                                Text.literal("This command can only be used in The Nether."));
                                        return 0;
                                    }
                                    ...
                                    CrackerMode crackerMode = CrackerMode
                                            .fromString(generationArg.toUpperCase(Locale.ROOT));
                                    if (crackerMode == null) {
                                        String validOptions = String.join(", ", GENERATION_TYPES);
                                        context.getSource().sendError(Text.literal("Invalid generation '"
                                                + generationArg + "'. Allowed types are: " + validOptions));
                                        return 0;
                                    }

                                    if (crackerService.isCrackingActive()) {
                                        context.getSource().sendError(Text.literal(
                                                "A cracking process is already running. Use /inbc stop first or /inbc info."));
                                        return 0;
                                    }

                                    IngameNetherBedrockCracker.LOGGER.info(
                                            "'/inbc start' command initiated. Type: {}, Threads: {}",
                                            crackerMode.name(), threadsArg);
                                    context.getSource().sendFeedback(Text
                                            .literal("Scanning for bedrock formations... This might take a moment."));

                                    CompletableFuture.runAsync(() -> {
                                        try {
                                            BlockPos playerPos = player.getBlockPos();
                                            List<BedrockFinder.FoundBedrock> foundRawBedrock = BedrockFinder
                                                    .findBedrockNearby(world, playerPos, false);

                                            MinecraftClient.getInstance().execute(() -> {
                                                if (foundRawBedrock.isEmpty()) {
                                                    context.getSource()
                                                            .sendError(Text.literal("No bedrock found at y=" +
                                                                    BedrockFinder.Y_LEVEL_FLOOR + " or y="
                                                                    + BedrockFinder.Y_LEVEL_CEILING +
                                                                    " within " + BedrockFinder.SEARCH_RADIUS_BLOCKS
                                                                    + " blocks."));
                                                    return;
                                                }

                                                IngameNetherBedrockCracker.LOGGER.info(
                                                        "Found {} bedrock positions. Preparing for cracking.",
                                                        foundRawBedrock.size());
                                                context.getSource()
                                                        .sendFeedback(Text.literal("Found " + foundRawBedrock.size()
                                                                + " bedrock positions. Starting cracker..."));

                                                List<BlockInput> blockInputs = new ArrayList<>();
                                                for (BedrockFinder.FoundBedrock bedrock : foundRawBedrock) {
                                                    blockInputs.add(new BlockInput(bedrock.x(), bedrock.y(),
                                                            bedrock.z(), bedrock_cracker_h.BlockType_BEDROCK()));
                                                }

                                                boolean started = crackerService.startCracking(blockInputs, crackerMode,
                                                        threadsArg);

                                                if (started) {
                                                    context.getSource().sendFeedback(Text.literal(
                                                            "Bedrock cracking process initiated. Use /inbc info for progress."));
                                                } else {
                                                    context.getSource().sendError(Text.literal(
                                                            "Failed to start cracking process. It might already be running or an internal error occurred. Check logs."));
                                                }
                                            });
                                        } catch (Exception e) {
                                            IngameNetherBedrockCracker.LOGGER.error(
                                                    "Error during bedrock scanning or crack initiation in StartCommand: ",
                                                    e);
                                            MinecraftClient.getInstance().execute(() -> context.getSource()
                                                    .sendError(Text.literal("An error occurred: " + e.getMessage())));
                                        }
                                    });
    ...
    private static CompletableFuture<Suggestions> suggestGenerations(
